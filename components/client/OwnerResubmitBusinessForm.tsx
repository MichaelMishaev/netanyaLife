...
import { resubmitPendingBusiness } from '@/lib/actions/business-owner'
...
interface OwnerResubmitBusinessFormProps {
  pendingId: string
  initialData: {
    name: string
...
export default function OwnerResubmitBusinessForm({
  pendingId,
  initialData,
  categories,
  neighborhoods,
  cityId,
  locale,
}: OwnerResubmitBusinessFormProps) {
  ...
  const [formData, setFormData] = useState({
    name_he: initialData.name || '',
    name_ru: '',
    categoryId: initialData.category_id || '',
    subcategoryId: initialData.subcategory_id || '',
    neighborhoodId: initialData.neighborhood_id || '',
    servesAllCity: initialData.serves_all_city || false,
    description_he: initialData.description || '',
    description_ru: '',
    phone: initialData.phone || '',
    whatsappNumber: initialData.whatsapp_number || '',
    websiteUrl: initialData.website_url || '',
    email: initialData.email || '',
    address_he: initialData.address || '',
    address_ru: '',
    openingHours_he: initialData.opening_hours || '',
    openingHours_ru: '',
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await resubmitPendingBusiness(pendingId, {
        name: formData.name_he,
        description: formData.description_he || undefined,
        ...
        opening_hours: formData.openingHours_he || undefined,
        address: formData.address_he || undefined,
        serves_all_city: formData.servesAllCity,
      })

      if (result.success) {
        // Track business resubmitted
    ...
    return (
      <div className="rounded-lg border border-green-200 bg-green-50 p-8 text-center">
        ...
        <h2 className="mb-2 text-2xl font-bold text-green-800">
          {locale === 'he' ? 'העסק נשלח שוב לאישור!' : 'Бизнес повторно отправлен на одобрение!'}
        </h2>
        <p className="text-green-700">
          {locale === 'he'
            ? 'העסק שלך עודכן ונשלח שוב לאישור מנהל. תקבל עדכון לאחר הבדיקה.'
            : 'Ваш бизнес обновлен и повторно отправлен на одобрение администратора. Вы получите обновление после проверки.'}
        </p>
      </div>
    )
  }
